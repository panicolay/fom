...
export function Track({
  ...
  const timeline: TimeLineItem[] = patterns
    ...
    : Array(totalBars)
        .fill(null)
        .map((_, index) => ({ type: 'empty', start: index }))
  ...
  return (
    ...
    >
      ...
      <button
        ...
        onClick={() => {
          onEdit(track, editTrackButtonRef.current)
        }}
        type="button"
      >
        {track.name}
      ...
      {/* Bars and patterns */}
      <div className="flex relative flex-grow hover:bg-base-900">
        {' '}
        {/** TODO: why relative? */}
        {timeline.map((item, index) => (
          <button
            ...
            onClick={() =>
              onPatternClick(track.id, item, patterns ?? [], patternButtonRefs.current[index])
            }
            type="button"
            style={{
        ...
        {currentEditingPattern && currentEditingPattern.track_id === track.id && (
          <div
            className="absolute top-0 h-10 bg-base-200 pointer-events-none animate-pulse"
            style={{
